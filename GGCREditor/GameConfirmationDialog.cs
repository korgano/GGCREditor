    ...
    public class GameConfirmationDialog : Form
    {
        ...
        public GameConfirmationDialog(GameDetector.DetectionResult result)
        {
            ...
            if (result != null && result.Profile != null)
            {
                ...
                {
                    for (int i = 0; i < cmbGameSelect.Items.Count; i++)
                    {
                        if (cmbGameSelect.Items[i] is KeyValuePair<string, string> kvp && kvp.Key == result.Profile.GameId)
                        {
                            cmbGameSelect.SelectedIndex = i;
                            break;
                        }
                    }
                }
